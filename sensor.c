...
//#ifdef USB_DEBUG_PRINT
	CyU3PDebugPrint (4, "\r\nThe Get control ID 0x%x %d", IDext, buf[0]); // additional debug
//#endif
	return buf[0];
};
...
//#ifdef USB_DEBUG_PRINT
	CyU3PDebugPrint (4, "\r\nThe Set control sensorAdd 0x%x boardAdd 0x%x devAdd 0x%x regAdd 0x%x 0x%x",
			SENSOR_ADDR_WR, I2C_DSPBOARD_ADDR_WR, devAdd, IDext, value); // additional debug
//#endif
	return 0;
};
/*************************************************************
